...
public partial class WindowsQuizViewModel : WindowsCardViewModel
{
    public WindowsQuizView qv;
    ...
    public override void SetView(WindowsCardView cv)
    {
        if (cv is WindowsQuizView _qv)
        {
            qv = _qv;
    ...
    {
        if (!CardQuestions[CardQuestions.IndexOf(CurrentQuestion)].Answered)
        {
            SetAnswered(1);
        }
        else
        ...
        if (answer)
        {
            ...
            if (Constants.LastQuiz != null)
            {
                ...
                if (result is bool boolResult)
                {
                    if (boolResult)
                    {
                        CardQuestions[CardQuestions.IndexOf(CurrentQuestion)].Locked = true;
                        await ReloadCurrentItem();
                        if (boolResult)
                        {
